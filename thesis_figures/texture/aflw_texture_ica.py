...
#    data=np.dot(X,filters.T)    
    ...
    rf=classifierUtils.standardRF(max_features = np.sqrt(len(testSet.data[0])),min_split=10,max_depth=40)
    if mode in ['s','v']:
        ...
        pickle.dump(rValues,open('texture_ica_{}'.format(components),'w'))
    elif mode in ['save']:
...
def _saveRF(testSet,rf,filters=None,meanI=None):
    ...
    pickle.dump(rf,open(root+'ica_50','w'))
    ...
    if filters is not None:
        np.save(root+'filter3',filters)
        np.save(root+'meanI3',meanI)
