            ...
            while True:
                if len(pref_styles) > 0:
                    ...
                    if pstyle not in rand_ls:
                        ...
                        ls=''.join(random.sample(colors,1))+'-'
...
def multiple_roc_average(rocList,binstep=.1):
    bins=np.arange(0,1+binstep,binstep)
    vals = [[] for i in range(len(bins)-1)]
    for roc in rocList:
        xy=[[x[0],x[1]] for x in roc.derived_points]
        XY = np.array(xy)
        inx = np.digitize(XY[:,0],bins)
        for i in range(1,len(bins)):
            vals[i-1].extend(XY[inx==i,1])
        
    return vals
    
def findEqualErrorRate(rocObject):
    data = np.array(rocObject.data)
    tp_count = np.count_nonzero(data[:,0]==1)
    tn_count = np.count_nonzero(data[:,0]==0)
    step = 0.01    
    ...
    for i,t in enumerate(thresholds):
        nfn=np.count_nonzero(np.logical_and(data[:,1]<t,data[:,0]==1))
        fnr[i]=(nfn*1./tp_count)
        nfp=np.count_nonzero(np.logical_and(data[:,1]>t,data[:,0]==0))
        fpr[i]=nfp*1./tn_count
        
    absdiff = np.abs(fnr-fpr)
    return thresholds[absdiff.argmin()]    
